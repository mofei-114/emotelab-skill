...
2. **新增任何部件后，必须同时确认对应的 `SlotGroup` 存在**。
   `Ahoge`、`Eyelash`、`SkinOutline` 都属于"JSON 里没有该组就回落到骨架 setup pose 颜色"
   （表现为白色或偏红）。加部件前先在这些角色的 SlotGroups 里搜一遍，
   或从 `SampleCharacter2.json` 抄组结构（游戏自带样例角色，本机在
...
**外套** `OutfitOuter1..5`：全是**泡泡袖短外套/披肩**（不透明，套上会盖住胸前的领结/吊带）。
